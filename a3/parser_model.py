...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.linear1 = nn.Linear(self.n_features*self.embed_size, hidden_size)
        print(f"Linear mat: {self.linear1.weight.shape}")
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(hidden_size, self.n_classes)

        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(self.linear1.weight))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(self.linear1.bias))
        self.dropout = nn.Dropout(self.dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(self.linear2.weight))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(self.linear2.bias))
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings.data, 0, torch.flatten(w)).view(-1, w.shape[1]*self.embed_size)               
    ...
    def forward(self, w):
        ...
        ### END YOUR CODE
        embed_input = self.embedding_lookup(w)
        hidden = self.linear1(embed_input)
        hidden_relu = self.relu(hidden)
        hidden_dropout = self.dropout(hidden_relu)
        logits = self.linear2(hidden_dropout)
